...
public sealed class CommodoreNewsImageRenderer : IDisposable
{
    private static readonly TimeSpan RedisTtl = TimeSpan.FromDays(7);
    private static readonly PetsciiatorOptions OnlineNewsOptions = new()
    {
    ...
    public async Task<byte[]> RenderAsync(string url, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                rendered = await _converter.ConvertAsync(bytes, OnlineNewsOptions, cancellationToken).ConfigureAwait(false);
    ...
    private static string BuildRedisKey(string url)
    {
        ...
        return $"bbs:commodorenews:petscii:v2:{hex}";
    }
    ...
    {
        Console.WriteLine($"[{DateTimeOffset.UtcNow:yyyy-MM-dd HH:mm:ss}][CommodoreNewsImageRenderer] {message}");
    }
}
